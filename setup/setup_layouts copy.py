...
# import setup.setup_utils as setup

# import pykiln_setup as pys

# from pykiln_setup import buttonClick
...
def buttonClick(newPage):
    ...
    if page == "connected":
        ...
        page = "complete"
        LayoutLoading("Getting IP Address", "You PyKiln is getting an IP address. Once it connects the ESP32 will reboot")
        return
    if page == "complete":
    ...
    if page == "fart":
        # LayoutTextImage(frame1, "Testing2", "Wow this is amazing content, the best content in the whole world!", "setup-unplug.gif", "Next!!!", "home")
        # LayoutText(frame1, "Testing", "Wow this is amazing content, the best content in the whole world!", "Next!!!", "home")
        LayoutText("Testing2", "Wow this is amazing content, the best content in the whole world!", "Next!!!", "home")
        return
...
def LayoutWifi(title, buttonText, pageName):
    ...
    e1 = Entry(container, width=50)
    e2 = Entry(container, width=50)
    e2.config(show=bullet)
    e3 = Entry(container, width=50)
...
def LayoutLoading(parent, title, content):
    # global page
    global headerFont
    global bodyFont
    global buttonFont
    ...
    # title
    title_text = Label(parent, text=title, font=headerFont)
    title_text.grid(row=0, column=0, columnspan=2, sticky="ew", pady=8, padx=16)

    # text content
    content_text = Label(parent, text=content, font=bodyFont, wraplength=300, justify="left", anchor="n")
    content_text.grid(row=1, column=0, sticky="new", pady=4, padx=16)
    ...
    Label(parent, text=" ").grid(row=4, sticky="s", pady=(250,0))
